...
from SACdemo.SACdemoImpl import SACdemo


class SACdemoTest(unittest.TestCase):

    @classmethod
    def setUpClass(cls):
        token = environ.get('KB_AUTH_TOKEN', None)
        cls.ctx = {'token': token, 'provenance': [{'service': 'SACdemo',
        ...
        for nameval in config.items('SACdemo'):
            cls.cfg[nameval[0]] = nameval[1]
        cls.wsURL = cls.cfg['workspace-url']
        cls.wsClient = workspaceService(cls.wsURL, token=token)
        cls.serviceImpl = SACdemo(cls.cfg)
    ...
    def getWsName(self):
        ...
        wsName = "test_SACdemo_" + str(suffix)
